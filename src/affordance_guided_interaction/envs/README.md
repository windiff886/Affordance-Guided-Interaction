...
```
         ...
         │  _get_observations()    Actor(858D)/Critic(874D) │
...
_get_observations()   ──▶  读取关节状态 + body 状态
                            ...
                            数值微分算加速度
                            计算 tilt proxy
                            拼接 Actor obs (858D) / Critic obs (874D)
                            缓存加速度/tilt_xy 供奖励使用
...
|---|---|---|
| `reset()` → `{"policy": (N, 858), "critic": (N, 874)}` | 解包为 per-env dict list | `(list[dict], list[dict])` |
...
| visual | Point-MAE door embedding(768) | 768 |
| **总计** | | **858** |
...
| 分段 | 内容 | 维度 |
|---|---|---|
| actor 部分 | 与 Actor 相同的 proprio + ee + context + stability + visual，但**无噪声** | 858 |
...
外部的 `PerceptionRuntime` 负责获取点云、运行 Point-MAE 推理，然后调用上述接口将 768 维 embedding 写入环境缓存。`visual_valid` 不再进入 Actor/Critic 观测；若视觉模块异常，运行时会直接报错终止。
...
| `update_visual_embedding(emb)` | PerceptionRuntime | 更新 768D Point-MAE 视觉 embedding |
