...
- **envs 只负责忠实地采集物理真值**，不做任何加工（差分估计加速度、拼接特征之类的事情是 `observations/` 的职责）
...
### 2.1 原始物理状态 → 供 `observations/` 构建观测
...
| 量 | 维度 | 说明 |
|---|---|---|
| 门板位姿 | (7,) | pos(3) + quat(4)，世界坐标系 |
| 门铰链角度 / 角速度 | (1,) + (1,) | 铰链关节的精确状态 |
| 杯体位姿 | (7,) | pos(3) + quat(4) |
| 杯体线速度 / 角速度 | (3,) + (3,) | 杯体的完整动力学状态 |
...
| 数据 | 说明 | 下游消费者 |
|---|---|---|
| 各 link 接触力幅值 | 机器人每个 link 受到的接触法向力大小 | `safety_penalty.py` 中的无效碰撞惩罚 $r_{\text{collision}}$ |
...
**为什么不让 rewards 层自己去读 Contact Sensor？** 因为 Isaac Sim 的原始接触数据频率极高、噪声大，需要先做阈值过滤和时间窗口平滑。这些物理层面的预处理属于环境封装的职责，而非奖励计算的职责。rewards 层只需要消费干净的布尔事件和标量力值。
...
- 根据当前课程阶段（curriculum stage）决定生成哪种类型的门（push / button / handle）
...
- `step(action)`：将动作映射为力矩，推进物理仿真一个 $\Delta t$，收集新状态，调用 ContactMonitor 和 TaskManager，返回观测、奖励数据源、done 信号
...
| 参数 | 含义 | 默认值 |
|---|---|---|
| $\theta_{\text{target}}$ | 推门成功角度阈值 | 1.2 rad（约 69°） |
...
### 为什么 envs 不直接构建 actor_obs / critic_obs？

因为观测构建涉及差分估计（加速度、jerk）、历史缓存（动作历史）和几何推理（倾斜度），这些计算逻辑与物理引擎的交互无关。把它们放在 envs 里会导致职责混杂——每次修改一个稳定性 proxy 的计算方式，就得去碰物理环境的代码。分层后两者可以独立迭代。
