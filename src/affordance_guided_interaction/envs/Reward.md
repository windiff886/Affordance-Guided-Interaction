...
> 规范超参数值定义在 `DoorPushEnvCfg`（`door_push_env_cfg.py`）中。
> 本文档中的超参数值为旧路径参考值，可能与 `DoorPushEnvCfg` 中的调优值不同。
...
每步的原始分项奖励分为正向激励（Bonus）和负向惩罚（Penalty）。当前课程设计中，Stage 1 完全不激活稳定性项；进入持杯阶段后，稳定性分支整体按原值计入总奖励，安全惩罚始终以原值计入总奖励：

$$
r_t = r_{\text{task}} + m_L \cdot \left( r_{\text{stab\_bonus}}^L + r_{\text{stab\_penalty}}^L \right) + m_R \cdot \left( r_{\text{stab\_bonus}}^R + r_{\text{stab\_penalty}}^R \right) - r_{\text{safe}}
$$
...
- Stage 1 中 $m_L = m_R = 0$，稳定性分支整体关闭；进入持杯课程阶段后，稳定性项全量生效
...
在第 2 章的总奖励公式中，单臂的稳定性项被明确划分为正向激励 $r_{\text{stab\_bonus}}$ 和负向惩罚 $r_{\text{stab\_penalty}}$ 两组；只要该侧处于持杯状态，这两部分就会一起生效。
...
| Stage 2 | 单臂持杯 | 持杯侧 Bonus + Penalty 全量生效 |
| Stage 3 | 混合分布 | 按具体上下文对持杯侧全量生效 |
