...
这一部分目前**暂定（to be determined）**。

现阶段只保留其功能性定义：Affordance 模块以上层视觉输入和任务目标为输入，并输出至少两类中间表示：

- **任务进展表示** $z_{\text{prog}}$
- **对象 affordance 表示** $z_{\text{aff}}$

但关于该模块的具体实现形式，目前仍需结合后续实验再决定，因此这里暂不对其网络结构、训练方式、监督形式和与低层策略的耦合方式做进一步限定。
